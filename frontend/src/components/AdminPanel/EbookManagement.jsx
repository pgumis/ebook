        ...
        <div>
            ...
            <table className="management-table">
                ...
                <tr>
                    ...
                    <th className="sortable" onClick={() => handleSort('tytul')}>Tytuł</th>
                    <th>Autor</th>
                    <th className="sortable" onClick={() => handleSort('cena')}>Cena</th>
                    <th className="sortable" onClick={() => handleSort('isbn')}>ISBN</th>
                    <th className="sortable" onClick={() => handleSort('kategoria')}>Kategoria</th>
                    <th className="sortable" onClick={() => handleSort('jezyk')}>Język</th>
                    <th className="sortable" onClick={() => handleSort('format')}>Format</th>
                    <th className="sortable" onClick={() => handleSort('data_wydania')}>Data wydania</th>
                    <th className="sortable" onClick={() => handleSort('status')}>Status</th>
                    <th>Akcje</th>
                </tr>
                </thead>
                <tbody>
                {loading && <tr><td colSpan="11">Odświeżanie...</td></tr>}
                {!loading && ebooks.length === 0 && (
                    <tr><td colSpan="11">Nie znaleziono e-booków pasujących do kryteriów.</td></tr>
                )}
                {!loading && ebooks.map((ebook) => (
                    <tr key={ebook.id}>
                        {/* ZAKTUALIZOWANE KOMÓRKI DANYCH */}
                        <td>{ebook.id}</td>
                        <td>{ebook.tytul}</td>
                        <td>{ebook.uzytkownik?.imie} {ebook.uzytkownik?.nazwisko}</td>
                        <td>{parseFloat(ebook.cena).toFixed(2)} zł</td>
                        <td>{ebook.isbn || '-'}</td>
                        <td>{ebook.kategoria || '-'}</td>
                        <td>{ebook.jezyk || '-'}</td>
                        <td>{ebook.format || '-'}</td>
                        <td>{ebook.data_wydania ? new Date(ebook.data_wydania).toLocaleDateString('pl-PL') : '-'}</td>
                        <td>
                        ...
                        <td>
                            {ebook.status !== 'aktywny' && (
                                <button className="action-btn activate-btn" onClick={() => handleStatusChange(ebook.id, 'aktywny')}>Aktywuj</button>
                            )}
                            {ebook.status === 'aktywny' && (
                                <button className="action-btn withdraw-btn" onClick={() => handleStatusChange(ebook.id, 'wycofany')}>Wycofaj</button>
                            )}
                        </td>
                    </tr>
                ))}
