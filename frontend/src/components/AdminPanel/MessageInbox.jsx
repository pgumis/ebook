        ...
        <div className="modal-backdrop">
            <div className="modal-content">
                ...
                <div className="message-meta">
                    ...
                    <strong>Data:</strong> {new Date(message.created_at).toLocaleString('pl-PL')}
                </div>
...
const MessageInbox = () => {
    ...
    const openMessage = async (message) => {
        ...
        if (message.przeczytana === 0) {
            try {
                ...
                await fetch(`http://localhost:8000/api/admin/wiadomosci/${message.id}/przeczytaj`, {
                    method: 'PUT',
    ...
    const handleFilterChange = (e) => { setReadFilter(e.target.value); setCurrentPage(1); };
    ...
    return (
        <div>
            ...
            <div className="filters-container">
                ...
                />
                <select className="filter-select" value={readFilter} onChange={handleFilterChange}>
            ...
            <table className="management-table">
                ...
                <tr>
                    {/* --- DODANA KOLUMNA LP. --- */}
                    <th>Lp.</th>
                    <th className="sortable" onClick={() => handleSort('imie')}>Nadawca</th>
                    <th className="sortable" onClick={() => handleSort('email')}>Email</th>
                    <th className="sortable" onClick={() => handleSort('temat')}>Temat</th>
                    <th className="sortable" onClick={() => handleSort('created_at')}>Data</th>
                    <th className="sortable" onClick={() => handleSort('przeczytana')}>Status</th>
                    <th>Akcje</th>
                </tr>
                </thead>
                <tbody>
                ...
                {!loading && messages.length === 0 && (
                    <tr><td colSpan="7">Brak wiadomości pasujących do kryteriów.</td></tr>
                )}
                {!loading && messages.map((message, index) => (
                    <tr key={message.id} className={message.przeczytana === 0 ? 'message-unread' : 'message-read'}>
                        {/* --- DODANA KOMÓRKA Z NUMEREM --- */}
                        <td>{startIndex + index + 1}</td>
                        <td>{message.imie}</td>
                        <td>{message.email}</td>
                        <td>{message.temat}</td>
                        <td>{new Date(message.created_at).toLocaleString('pl-PL')}</td>
                        <td>
                            <span className={`status-badge ${message.przeczytana === 0 ? 'status-nieprzeczytane' : 'status-przeczytane'}`}>
                                {message.przeczytana === 0 ? 'Nieprzeczytane' : 'Przeczytane'}
                            </span>
                        </td>
                        <td>
                            <button className="action-btn view-btn" onClick={() => openMessage(message)}>Zobacz</button>
                            <button className="action-btn delete-btn" onClick={() => handleDeleteMessage(message.id)}>Usuń</button>
                        </td>
                    </tr>
                ))}
                </tbody>
