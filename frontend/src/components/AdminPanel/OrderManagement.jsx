...
const OrderManagement = () => {
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    try {
      await fetch(
        `http://localhost:8000/api/admin/zamowienia/${orderId}/status`,
        {
          method: "PUT",
          headers: {
        ...
        <table className="management-table">
          <thead>
            <tr>
              ...
              <th className="sortable" onClick={() => handleSort("suma")}>
                Suma
              </th>
            ...
            {!loading &&
              orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td>
                    {order.uzytkownik
                      ? `${order.uzytkownik.imie} ${order.uzytkownik.nazwisko}`
                      : "Użytkownik usunięty"}
                  </td>
