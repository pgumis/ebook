        ...
        <table className="management-table">
          <thead>
            <tr>
              ...
              <th className="sortable" onClick={() => handleSort("ocena")}>
                Ocena
              </th>
              <th>Treść</th>
          ...
          <tbody>
            ...
            {!loading && reviews.length === 0 && (
              <tr>
                <td colSpan="7">
                  Nie znaleziono recenzji pasujących do kryteriów.
                </td>
              </tr>
            )}
            {!loading &&
              reviews.map((review) => (
                <tr key={review.id}>
                  ...
                  <td>
                    {review.uzytkownik?.imie}{" "}
                    {review.uzytkownik?.nazwisko || "Brak danych"}
                  </td>
                  <td>{generateStars(review.ocena)}</td>
                  <td className="review-content">
                    {review.tresc
                      ? `${review.tresc.substring(0, 100)}...`
                      : "-"}
                  </td>
                  <td>
